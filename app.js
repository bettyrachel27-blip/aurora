...
function renderRoutines(){const root=$("#routineContent"),items=visibleHabits();$("#routineModeLabel").textContent=dayLabel[state.dayMode];if(routineTab==="today"){const groups=[...new Set(items.map(x=>x.group))];root.innerHTML=groups.map(g=>`<div class="paper-card"><div class="section-title"><h3>${g}</h3></div>${items.filter(x=>x.group===g).map(habitMarkup).join("")}</div>`).join("")}else if(routineTab==="stats"){const done=items.filter(x=>x.done).length,p=items.length?Math.round(done/items.length*100):0;root.innerHTML=`<div class="summary-card summary-card--compact"><span class="summary-card-title">Régularité</span><div class="progress-donut" style="--progress:${p}%"><span>${p}%</span><small>Aujourd’hui</small></div><ul><li><i class="row-icon">✓</i><b>Réalisées</b><strong>${done}</strong></li><li><i class="row-icon">○</i><b>Restantes</b><strong>${items.length-done}</strong></li></ul></div>`}else{root.innerHTML=`<div class="paper-card"><div class="row"><div class="row-main"><b>💼 Routine Travail</b><small>Hydratation, organisation, préparation du lendemain.</small></div><button class="brown-button" data-profile="work">Utiliser</button></div><div class="row"><div class="row-main"><b>🏡 Routine Repos</b><small>Sport, balade, courses et projets.</small></div><button class="brown-button" data-profile="rest">Utiliser</button></div><div class="row"><div class="row-main"><b>✈️ Routine Vacances</b><small>Hydratation, dépenses voyage et souvenirs.</small></div><button class="brown-button" data-profile="vacation">Utiliser</button></div></div>`}bindRows();$$("[data-profile]").forEach(b=>b.onclick=()=>{state.dayMode=b.dataset.profile;save();routineTab="today";render()})}
...
function go(screen){$$(".screen").forEach(s=>s.classList.toggle("active",s.dataset.screen===screen));$$(".bottom-nav [data-go]").forEach(b=>b.classList.toggle("active",b.dataset.go===screen));$$(".sidebar-nav [data-go]").forEach(b=>b.classList.toggle("active",b.dataset.go===screen));$("#sideMenu").classList.remove("open");if($("#settingsDayMode"))$("#settingsDayMode").textContent=dayLabel[state.dayMode];window.scrollTo({top:0,behavior:"smooth"})}
