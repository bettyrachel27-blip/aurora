...
$("#waterMinus").onclick=()=>{state.water=Math.max(0,Math.round((state.water-.25)*100)/100);save();render()};
$("#waterPlus").onclick=()=>{state.water=Math.min(6,Math.round((state.water+.25)*100)/100);save();render();toast("Un verre d’eau ajouté 💧")};
$("#stepsMinus").onclick=()=>{state.steps=Math.max(0,state.steps-500);save();render()};
$("#stepsPlus").onclick=()=>{state.steps=Math.min(100000,state.steps+500);save();render();toast("500 pas ajoutés ✨")};
