...
let state=load(),agendaMode="day",taskTab="all",routineTab="today",statsPeriod="week",activeTaskId=null;
...
function render(){
 ...
 renderPageContexts();renderRhythm();renderAgenda();renderTasks();renderRoutines();renderBudget();renderProjects();renderNotifications();renderStats();
}
...
function renderAgenda(){
 ...
 if(agendaMode==="day"){
   ...
   const dayEvents=events.filter(e=>e.date===state.selectedDate).sort((a,b)=>(a.type===preferred?0:1)-(b.type===preferred?0:1)||a.time.localeCompare(b.time));
   ...
   root.innerHTML=weekDates.map(d=>{const preferred=state.dayMode==="work"?"work":"personal";const de=events.filter(e=>e.date===d).sort((a,b)=>(a.type===preferred?0:1)-(b.type===preferred?0:1)||a.time.localeCompare(b.time));const dt=tasks.filter(t=>t.due===d&&!t.done);return `<section class="week-day ${d===isoToday()?"today":""}"><div class="week-day-head"><b>${fmt(d,{weekday:"long"})}</b><small>${fmt(d,{day:"numeric",month:"short"})}</small></div>${de.map(e=>`<span class="week-item ${e.type}"><b>${e.time}</b> ${e.title}</span>`).join("")}${dt.map(t=>`<span class="week-item task ${t.type}">✓ ${t.title}</span>`).join("")}${!de.length&&!dt.length?'<small style="opacity:.55">Journée libre</small>':''}</section>`}).join("");
 }else{
...
function renderRoutines(){const root=$("#routineContent"),items=visibleHabits();$("#routineModeLabel").textContent=dayLabel[state.dayMode];if(routineTab==="today"){const groups=[...new Set(items.map(x=>x.group))];root.innerHTML=groups.map(g=>`<div class="paper-card"><div class="section-title"><h3>${g}</h3></div>${items.filter(x=>x.group===g).map(habitMarkup).join("")}</div>`).join("")}else if(routineTab==="stats"){const done=items.filter(x=>x.done).length,p=items.length?Math.round(done/items.length*100):0;root.innerHTML=`<div class="summary-card"><div><span>Régularité</span><ul><li><b>Réalisées</b><strong>${done}</strong></li><li><b>Restantes</b><strong>${items.length-done}</strong></li></ul></div><div class="progress-donut" style="--progress:${p}%"><span>${p}%</span><small>Aujourd’hui</small></div></div>`}else{root.innerHTML=`<div class="paper-card"><div class="row"><div class="row-main"><b>💼 Routine Travail</b><small>Hydratation, organisation, préparation du lendemain.</small></div><button class="brown-button" data-profile="work">Utiliser</button></div><div class="row"><div class="row-main"><b>🏡 Routine Repos</b><small>Sport, balade, courses et projets.</small></div><button class="brown-button" data-profile="rest">Utiliser</button></div><div class="row"><div class="row-main"><b>🌙 Routine Récupération</b><small>Repos, hydratation et mouvements doux.</small></div><button class="brown-button" data-profile="recovery">Utiliser</button></div><div class="row"><div class="row-main"><b>✈️ Routine Vacances</b><small>Hydratation, dépenses voyage et souvenirs.</small></div><button class="brown-button" data-profile="vacation">Utiliser</button></div><div class="row"><div class="row-main"><b>🌿 Routine Repos santé</b><small>Repos, eau et suivi du bien-être.</small></div><button class="brown-button" data-profile="sick">Utiliser</button></div></div>`}bindRows();$$("[data-profile]").forEach(b=>b.onclick=()=>{state.dayMode=b.dataset.profile;save();routineTab="today";render()})}
...
function reportTask(t,mode){const base=t.due&&t.due>isoToday()?t.due:isoToday();if(mode==="tomorrow"){const d=new Date(base+"T12:00:00");d.setDate(d.getDate()+1);t.due=d.toISOString().slice(0,10)}else if(mode==="next-workday")t.due=nextWorkday(base);else if(mode==="next-free-day")t.due=nextFreeDay(base);t.done=false;save();render();$("#taskActionDialog").close();toast("Tâche reportée au "+fmt(t.due,{day:"numeric",month:"long"}))}
...
if(enterAurora&&welcomeScreen){
  enterAurora.addEventListener("click",()=>{
    ...
    sessionStorage.setItem("aurora-1.8-entered","yes");
  });
}
// L'écran d'entrée réapparaît à chaque nouvelle session, mais pas lors d'une simple navigation interne.
if(sessionStorage.getItem("aurora-1.8-entered")==="yes"&&welcomeScreen){welcomeScreen.classList.add("is-hidden")}
...
if("serviceWorker"in navigator){
  navigator.serviceWorker.register("service-worker.js?v=160").then(reg=>reg.update()).catch(()=>{});
}